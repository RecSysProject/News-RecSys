...
n_sim_user = 10      #相似用户数量       *重要5，10，20，40，(80,0.1406)最好，100
n_rec_news = 10     #推荐电影数量

user_sim_mat = {}    #用户间的item相似度矩阵
news_popular = {}   #item ：流行度
news_count = 0

print ('Similar user number = %d' % n_sim_user, file=sys.stderr)
...
news_count = len(news2users)  ## item 总数 2194 保存用于评估
...
for u, related_users in usersim_mat.items():
    for v, count in related_users.items():
        usersim_mat[u][v] = count / math.sqrt(len(trainset[u]) * len(trainset[v]))
        simfactor_count += 1
...
def recommend(user):
    ...
    for similar_user, similarity_factor in sorted(user_sim_mat[user].items(),
        ...
        "遍历这些相似用户看过的所有item"
        for new in trainset[similar_user]:
...
print ('K=%d precision=%.4f\trecall=%.4f\tcoverage=%.4f\tpopularity=%.4f' % (n_sim_user, precision, recall, coverage, popularity), file=sys.stderr)
