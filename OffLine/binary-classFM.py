...
if __name__ == '__main__':
    ...
    w_0, w, v = test.SGD_FM(np.mat(X_train_gb), y_train_gb, 20, 60)
    print("训练准确性为：%f" % (1 - test.getAccuracy(np.mat(X_train_gb), y_train_gb, w_0, w, v)))

    Train_end = datetime.now()
    print("训练用时为：%s" % (Train_end - Train_start))

    print("开始测试")
    print("测试准确性为：%f" % (1 - test.getAccuracy(np.mat(X_test_gb), y_test_gb, w_0, w, v)))
