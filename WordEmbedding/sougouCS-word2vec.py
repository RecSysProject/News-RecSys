...
class wordEmbedding(object):
    ...
    '训练生成词向量'
    def model_train(self, train_file_name, save_model_file):
        ...
        model.wv.save_word2vec_format(save_model_name+".bin", binary=True)# 二进制类型保存以便重用
    
    def modeling(self):
        train_file = self.root + "sougouCS_corpus.txt"
        save_model_name = self.root + 'sougouCS_wordVec'
        if not os.path.exists(save_model_name):     # 判断文件是否存在
            self.model_train(train_file, save_model_name)
        else:
